...
const PORTFOLIO_TAGS = ['EMBLEMAS', 'ARTISTAS', 'MARCAS', 'ARTWORK']
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="flex justify-between text-[10px] text-brand-600"><span>Invisible (0%)</span><span>Pleno (100%)</span></div>
      </div>
      <div>
        <label className="block text-xs text-brand-500 mb-1">Vignette (oscurecimiento bordes): {Math.round(((hero.vignetteOpacity as number) ?? 0.7) * 100)}%</label>
        <input type="range" min={0} max={100} step={5} value={Math.round(((hero.vignetteOpacity as number) ?? 0.7) * 100)} onChange={(e) => { hero.vignetteOpacity = Number(e.target.value) / 100; update() }} className="w-full accent-white h-1.5" />
        <div className="flex justify-between text-[10px] text-brand-600"><span>Sin vignette (0%)</span><span>Máxima (100%)</span></div>
      </div>
